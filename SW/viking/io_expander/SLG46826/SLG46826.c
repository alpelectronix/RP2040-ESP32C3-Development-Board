...
bool readSlg(uint8_t controlCode, uint8_t data[256], uint8_t nvm) {
...
bool writeSlg(uint8_t controlCode, const uint8_t data[256], uint8_t mode) {
...
void writeReg(uint8_t address, uint8_t reg, uint8_t data) {
    log_v("writeReg(0x%02X, 0x%02X, 0x%02X)", reg, data);
...
void writeRegEx(uint8_t address, uint8_t reg, const uint8_t *data, uint8_t size) {
    log_v("writeRegEx(0x%02X, 0x%02X, *, %d)", reg, size);
    //uint32_t clock = _i2cWire->getClock();
    // _i2cWire->setClock(100000);
    // _i2cWire->beginTransmission(deviceAddress);
    // _i2cWire->write(reg);
